...
cat("model {
    ...
    omega ~ dunif(0,1) 
    psiMean ~ dunif(0,1) # Occurrence probability
    
    for (t in 1:T) { # T = number of years sampled. 
    pMean[t] ~ dunif(0,1) # Year-specific detection probability
    }
    
    for (t in 1:(T-1)) {
    phiMean[t] ~ dunif(0,1) # Year-specific persistence probability
    gamMean[t] ~ dunif(0,1) # Year-specific colonization probability
    }
    #-------------------------------------------------------------------------------
    ...
    w[i] ~ dbern(omega) # Species- and year- specific probability of being included in the 
    # metacommunity
    ...
    mu.z[i,k,t+1] <- w[i] * (phi[i,k,t] * z[i,k,t] + gam[i,k,t] * (1-z[i,k,t]))
